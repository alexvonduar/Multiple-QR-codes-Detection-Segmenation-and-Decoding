...
int OpenQR::GetCoresSize()
{
    ...
    numOfCores = OMP_NUM_THREADS;
    printf("OMP: OPENMP_NUM_THREADS: %d\n", numOfCores);
#endif

    int max_threads = omp_get_max_threads();
    if (numOfCores == 0)
    {
        numOfCores = max_threads;
        printf("OMP: set num threads to max threads: %d\n", numOfCores);
    }
    if (max_threads < numOfCores)
    {
    ...
    if (numOfCores == 0)
    {
        numOfCores = num_procs;
        printf("OMP: set num threads to num procs: %d\n", numOfCores);
    }
    if (num_procs < numOfCores)
    {
        printf("OMP: max processors %d\n", num_procs);
        numOfCores = num_procs;
    }
    return numOfCores;
#else
...
int OpenQR::OpenCamera(int width, int height)
{
    ...
    videoStream = new cv::VideoCapture();
    ...
    videoStream->open(deviceID, apiID);
...
int OpenQR::ReadFrame(const std::string &fname)
{
    if (fname.length())
    {
        imgSource = cv::imread(fname);
    }
    else
    {
...
QrCodePose OpenQR::GetQrCodePose(QrCodePose &qrcode, cv::Mat imgBin, int &flag)
{
    ...
    angleRef[0] = {
        qrcode.ptTopL,
        cv::Point(0, 0),
        GetRotatedAngle(qrcode.ptMid, qrcode.ptTopL),
    };
...
#pragma omp for
        for (int i = 0; i < inlineContours.size(); i++)
        {
            ...
            if (fabs(cv::contourArea(approximatedContours)) > 20 && approximatedContours.size() == 4)
            {
                ...
                tmp.length = (cv::norm(approximatedContours[0] - approximatedContours[1]) + cv::norm(approximatedContours[1] - approximatedContours[2]) + cv::norm(approximatedContours[2] - approximatedContours[3]) + cv::norm(approximatedContours[3] - approximatedContours[0])) / 4.0;
...
#pragma omp for
        ...
        {
            cv::Rect tmp;
            tmp.x = std::min(std::min(expectedQrCodes[i].ptTopL.x, expectedQrCodes[i].ptTopR.x), std::min(expectedQrCodes[i].ptBotR.x, expectedQrCodes[i].ptBotL.x));
            tmp.y = std::min(std::min(expectedQrCodes[i].ptTopL.y, expectedQrCodes[i].ptTopR.y), std::min(expectedQrCodes[i].ptBotR.y, expectedQrCodes[i].ptBotL.y));
            int maxX = std::max(std::max(expectedQrCodes[i].ptTopL.x, expectedQrCodes[i].ptTopR.x), std::max(expectedQrCodes[i].ptBotR.x, expectedQrCodes[i].ptBotL.x));
            int maxY = std::max(std::max(expectedQrCodes[i].ptTopL.y, expectedQrCodes[i].ptTopR.y), std::max(expectedQrCodes[i].ptBotR.y, expectedQrCodes[i].ptBotL.y));
